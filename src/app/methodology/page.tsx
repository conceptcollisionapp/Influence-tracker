    ...
    <article className="prose-invert mx-auto max-w-3xl space-y-10">
      <section>
        ...
        <ul className="mt-3 list-disc space-y-2 pl-6 text-slate-300">
          ...
          <li>
            <span className="rounded-full border border-line bg-surface px-2 py-0.5 text-xs text-slate-400">verified</span>{" "}
      ...
      <section>
        ...
        <div className="space-y-4">
          {SOURCES.map((s) => (
            <div key={s.name} className="rounded-xl border border-line bg-surface/70 p-4">
              <a href={s.url} target="_blank" rel="noopener noreferrer" className="font-semibold text-brand-soft underline">
                {s.name} ↗
              </a>
        ...
        <div className="mt-3 overflow-hidden rounded-xl border border-line">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="text-slate-300">
              <tr className="border-t border-line"><td className="px-4 py-2">Wealth</td><td className="px-4 py-2">40</td><td className="px-4 py-2">Net worth, log-scaled from the $10M tracking floor to a $500B ceiling. Live Wikidata value when available, otherwise the curated estimate.</td></tr>
              <tr className="border-t border-line"><td className="px-4 py-2">Network</td><td className="px-4 py-2">25</td><td className="px-4 py-2">Degree in the documented connection graph (both directions) plus breadth of organizational footprint.</td></tr>
              <tr className="border-t border-line"><td className="px-4 py-2">Political</td><td className="px-4 py-2">20</td><td className="px-4 py-2">Control of PACs/candidate committees plus disclosed FEC giving volume (log-scaled, fetched live).</td></tr>
              <tr className="border-t border-line"><td className="px-4 py-2">Institutional</td><td className="px-4 py-2">15</td><td className="px-4 py-2">Control of public companies, major nonprofits, and media properties in the registry.</td></tr>
            </tbody>
          </table>
        </div>
      ...
      <section>
        ...
        <ul className="list-disc space-y-2 pl-6 text-sm text-slate-300">
          ...
          <li>Found an error? Every datum links to its source — corrections to the registry are a one-line change in <code className="rounded bg-surface-2 px-1">src/lib/people.ts</code>.</li>
        </ul>
      </section>
    </article>
  );
